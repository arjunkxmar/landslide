...
export default function PredictionPage({ onOpenEmergencyModal }) {
  ...
  const loadLiveWeather = useCallback(async (lat, lng, locationLabel, autoApply = true) => {
    ...
    try {
      ...
      if (autoApply) {
        // Feed live API telemetry directly into environmental inputs
        ...
        setFormData(prev => {
          ...
          // Immediately compute risk prediction using project's geotechnical engine
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-800 pb-4">
        <div>
          <div className="flex items-center gap-2">
            <span className="text-xs font-mono text-cyan-400 font-bold uppercase tracking-wider">
              Geotechnical Machine Learning Prototype
            </span>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto flex-wrap">
          ...
          <button
            ...
            className="flex items-center gap-1.5 px-3.5 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-300 hover:text-white text-xs font-mono hover:border-slate-700 transition-colors cursor-pointer"
          >
          ...
          <button
            onClick={handleReset}
            className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl bg-slate-900 border border-slate-800 text-slate-400 hover:text-white text-xs font-mono hover:border-slate-700 transition-colors cursor-pointer"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-3 items-center">
          ...
          <div className="md:col-span-6">
            <select
              ...
              className="w-full px-3.5 py-2 bg-slate-900 border border-slate-700 rounded-xl text-xs font-mono text-white focus:outline-none focus:border-cyan-400 cursor-pointer"
            >
          ...
          {isCustomLocation ? (
            <div className="md:col-span-6 flex items-center gap-2">
              <input
                ...
                className="w-1/2 px-3 py-1.5 bg-slate-900 border border-slate-700 rounded-xl text-xs font-mono text-white"
              />
              <input
                type="number"
                step="0.0001"
              ...
              <button
                type="button"
                onClick={() => loadLiveWeather(customCoords.lat, customCoords.lng, customCoords.name, true)}
                className="px-3 py-1.5 rounded-xl bg-cyan-500 hover:bg-cyan-400 text-slate-950 font-bold text-xs font-mono shrink-0 cursor-pointer"
              >
                Fetch
              </button>
            </div>
          ) : (
            <div className="md:col-span-6 flex items-center justify-between text-xs font-mono text-slate-400 bg-slate-900/60 px-3.5 py-2 rounded-xl border border-slate-800">
              <span>Coordinates: {activeLat.toFixed(4)}°N, {activeLng.toFixed(4)}°E</span>
              <span className="text-cyan-400">{activeStation.region}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="p-4 rounded-2xl glass-panel border border-slate-800 space-y-2">
        <div className="flex items-center justify-between text-xs font-mono text-slate-400">
          <span className="uppercase font-bold tracking-wider">Geotechnical Stress-Testing Benchmarks:</span>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2.5">
          {PRESET_SCENARIOS.map((preset, idx) => (
            <button
              ...
              className="p-2.5 rounded-xl bg-slate-900/80 border border-slate-800 hover:border-cyan-500/50 hover:bg-slate-900 text-left transition-all group cursor-pointer"
            >
      ...
      {compareMode && (
        <div className="p-4 rounded-2xl bg-purple-950/40 border border-purple-500/40 animate-fadeIn space-y-3">
          <div className="flex items-center justify-between text-xs font-mono">
            <span className="text-purple-300 font-bold uppercase flex items-center gap-1.5">
              <GitCompare className="w-4 h-4 text-purple-400" />
              What-If Comparative Scenario: Live Telemetry vs. Severe Cloudburst (+53 mm)
            </span>
            <span className="text-purple-400 font-bold">Delta: +{compareResult.score - analysisResult.score}% Threat Surge</span>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div className="p-3 rounded-xl bg-slate-900/90 border border-slate-800 flex items-center justify-between">
              <div>
                ...
                <span className="text-xl font-bold font-mono text-white">{analysisResult.score}%</span>
              </div>
              <RiskBadge level={analysisResult.riskInfo.level} score={analysisResult.score} size="md" />
            </div>

            <div className="p-3 rounded-xl bg-purple-900/40 border border-purple-500/50 flex items-center justify-between">
              <div>
                <span className="text-[10px] font-mono text-purple-300 block uppercase">Storm Cloudburst (+53mm)</span>
                <span className="text-xl font-bold font-mono text-rose-400">{compareResult.score}%</span>
              </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
        ...
        <div className="lg:col-span-6 p-6 rounded-2xl glass-panel border border-slate-800 space-y-6">
          <div className="flex items-center justify-between border-b border-slate-800 pb-3">
            <h3 className="text-base font-bold text-white flex items-center gap-2">
              <Sliders className="w-4 h-4 text-cyan-400" />
              <span>Multi-Factor Environmental Parameters</span>
            </h3>
            <span className="text-xs font-mono text-cyan-400 bg-cyan-500/10 px-2 py-0.5 rounded-full border border-cyan-500/30">
              Live Synchronized
            </span>
          </div>
          ...
          <div className="space-y-5">
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between text-xs">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div className="font-mono font-bold text-white text-sm bg-slate-900 px-2.5 py-0.5 rounded-md border border-slate-800">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-2">
                <div className="flex items-center justify-between text-xs">
                  ...
                  <div className="font-mono font-bold text-white text-xs bg-slate-900 px-2 py-0.5 rounded border border-slate-800">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
                {[
                  ...
                  <button
                    key={item.id}
                    type="button"
                    onClick={() => handleInputChange('history', item.id)}
            ...
            <div className="pt-3">
              <button
                ...
                className="w-full py-4 rounded-2xl bg-gradient-to-r from-purple-600 via-indigo-600 to-cyan-600 hover:from-purple-500 hover:via-indigo-500 hover:to-cyan-500 text-white font-black tracking-wide text-base shadow-[0_0_30px_rgba(139,92,246,0.35)] transition-all transform active:scale-[0.99] disabled:opacity-50 cursor-pointer flex items-center justify-center gap-3"
              >
                {isAnalyzing ? (
                  <>
            ...
            <div className={`p-6 rounded-2xl border transition-all duration-500 ${
              ...
              <div className="space-y-3 mt-6 pt-5 border-t border-slate-800">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="p-3 rounded-xl bg-slate-900/80 border border-slate-800">
                    <div className="flex justify-between text-xs font-mono mb-1">
                      <span className="text-slate-400">Historical Fault</span>
              ...
              <div className="mt-5 p-4 rounded-xl bg-slate-900/90 border border-slate-700 space-y-2">
              ...
              <div className="mt-5 flex gap-3">
                <button
                  ...
                  className="w-full py-3 rounded-xl bg-rose-600 hover:bg-rose-500 text-white font-bold text-xs shadow-lg shadow-rose-950/40 flex items-center justify-center gap-2 transition-all cursor-pointer"
                >
              ...
              <div className="mt-4 p-2.5 rounded-lg bg-slate-900/50 border border-slate-800/80 text-[11px] text-slate-400 flex items-start gap-2 font-mono">
